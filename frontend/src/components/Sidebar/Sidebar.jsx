...
export default function Sidebar({
  ...
  const handleSidebarReportDelete = async (reportId) => {
    if (!window.confirm("Are you sure you want to delete this report record?")) return;
    try {
      const token = safeStorage.getLocal("ai_doctor_token");
      const res = await fetch(`/api/patient/reports/${reportId}`, {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${token}`
        }
      ...
      if (res.ok) {
        ...
        if (selectedSavedReport && (selectedSavedReport._id === reportId || selectedSavedReport.id === reportId)) {
